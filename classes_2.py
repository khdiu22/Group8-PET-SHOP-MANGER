...
class PetShop:
    ...
    def find_pet(self, pet_name): 
        for pet in self.available_pets + self.sold_pets: 
            if pet.get_name().lower() == pet_name.lower(): # الحصول على اسم الحيوان وتحويله لحروف صغيره
                return pet # يعيد كائن الحيوان بالكامل
        return None 
    ...
    def sell_pet(self, customer_id, pet_name):
        ...
        customer = self.customers[customer_id] #جلب العميل للكائن نفسه
        
        pet.is_available = False #تحديث حاله الحيوان
    ...
    def filter_by_type(self, species): 
        ...
        for pet in self.available_pets:
            if pet.species.lower() == species.lower():# الشرط يتحقق إذا كان نوع الحيوان يساوي النوع المطلوب، بدون ما نهتم بحالة الأحرف
                found = True
                match pet.species:
                    case "Dog":
                        print("Dog available:")
